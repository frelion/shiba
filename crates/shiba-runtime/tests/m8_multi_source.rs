...
fn continuations(client: &mut Client) -> Vec<(i64, i64, i64)> {
    client
        .query(
            "SELECT source_id, slot_generation, count(*)
             FROM shiba_internal.source_continuation
             GROUP BY source_id, slot_generation
             ORDER BY source_id",
            &[],
        )
...
fn m8_two_sources_have_independent_continuation_and_recovery() {
    ...
    let transaction1 =
        decode_committed_changes(&source1_wire, source1).expect("decode source1 transaction");
    client
    ...
    let transaction2 =
        decode_committed_changes(&source2_wire, source2).expect("decode source2 transaction");
    assert_eq!(
    ...
    let recovery =
        decode_committed_changes(&recovery_wire, source2).expect("decode source2 recovery");
    ...
    let drift =
        decode_committed_changes(&recovery_wire, generation2).expect("decode generation drift");
    assert!(matches!(
