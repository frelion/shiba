#!/usr/bin/env bash
# M2 integration gate. It owns one disposable PostgreSQL cluster and exposes
# only its connection string to the Rust integration test.
set -euo pipefail
...
tmp="$(mktemp -d "/tmp/shiba-m2-pg${pg_major}.XXXXXX")"
data="$tmp/data"
socket="$tmp/socket"
package="$tmp/package"
port="$(python3 - <<'PY'
...
SHIBA_M2_DATABASE_URL="host=$socket port=$port dbname=postgres user=$(id -un)" \
  PG_CONFIG="$pg_config" \
  cargo test -p shiba-runtime --test m2_postgres -- --ignored --test-threads=1

echo "M2 integration passed for PostgreSQL $pg_major ($pg_config)"
