...
tmp="$(mktemp -d "/tmp/shiba-m4-update-pg${pg_major}.XXXXXX")"
data="$tmp/data"
socket="$tmp/socket"
package="$tmp/package"
capture="$tmp/capture"
...
SHIBA_M4_UPDATE_DATABASE_URL="host=$socket port=$port dbname=postgres user=$(id -un)" \
SHIBA_M4_UPDATE_PG_BINDIR="$bindir" \
SHIBA_M4_UPDATE_HOST="$socket" \
SHIBA_M4_UPDATE_PORT="$port" \
SHIBA_M4_UPDATE_USER="$(id -un)" \
SHIBA_M4_UPDATE_CAPTURE_DIR="$capture" \
PG_CONFIG="$pg_config" \
  cargo test -p shiba-runtime --test m4_update -- --ignored --test-threads=1

echo "M4 update integration passed for PostgreSQL $pg_major ($pg_config)"
