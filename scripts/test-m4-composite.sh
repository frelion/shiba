...
tmp="$(mktemp -d "/tmp/shiba-m4-composite-pg${pg_major}.XXXXXX")"
data="$tmp/data"
socket="$tmp/socket"
package="$tmp/package"
capture="$tmp/capture"
...
SHIBA_M4_COMPOSITE_DATABASE_URL="host=$socket port=$port dbname=postgres user=$(id -un)" \
SHIBA_M4_COMPOSITE_PG_BINDIR="$bindir" \
SHIBA_M4_COMPOSITE_HOST="$socket" \
SHIBA_M4_COMPOSITE_PORT="$port" \
SHIBA_M4_COMPOSITE_USER="$(id -un)" \
SHIBA_M4_COMPOSITE_CAPTURE_DIR="$capture" \
PG_CONFIG="$pg_config" \
  cargo test -p shiba-runtime --test m4_composite -- --ignored --test-threads=1

echo "M4 composite integration passed for PostgreSQL $pg_major ($pg_config)"
