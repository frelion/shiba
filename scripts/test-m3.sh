...
tmp="$(mktemp -d "/tmp/shiba-m3-pg${pg_major}.XXXXXX")"
data="$tmp/data"
socket="$tmp/socket"
package="$tmp/package"
capture="$tmp/capture"
...
SHIBA_M3_DATABASE_URL="host=$socket port=$port dbname=postgres user=$(id -un)" \
SHIBA_M3_PG_BINDIR="$bindir" \
SHIBA_M3_HOST="$socket" \
SHIBA_M3_PORT="$port" \
SHIBA_M3_USER="$(id -un)" \
SHIBA_M3_CAPTURE_DIR="$capture" \
PG_CONFIG="$pg_config" \
  cargo test -p shiba-runtime --test m3_pgoutput -- --ignored --test-threads=1

echo "M3 integration passed for PostgreSQL $pg_major ($pg_config)"
