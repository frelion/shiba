...
# Keep the accepted M4.1 production budget executable rather than aspirational.
python3 - <<'PY'
import pathlib
...
if too_large:
    raise SystemExit("M4.1 production file exceeds 250 lines: " + ", ".join(too_large))
if sum(line_counts.values()) > 600:
    raise SystemExit("M4.1 Runtime production code exceeds its 600-line hard limit")
sql_counts = {
...
if too_large_sql:
    raise SystemExit("M4.1 SQL file exceeds 150 lines: " + ", ".join(too_large_sql))
PY
...
for required in ("Protocol JSON/schema", "canonical digest", "PG17/18", "Phase 1", "M3.1", "M3.2", "M4.1"):
