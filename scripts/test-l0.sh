...
scripts = [
    ...
    for name in (
        ...
        "m6-stream-commit", "m6-stream-abort",
    )
]
for path in scripts:
    text = path.read_text()
...
for required in ("Protocol JSON/schema", "canonical digest", "PG17/18", "Phase 1", "M3.1", "M3.2", "M4.1", "M4.2", "M4.3", "M4.4", "M4.5", "M4.6", "M5.1", "M5.2", "M5.3", "M5.4", "M5.5", "M6.1", "M6.2"):
