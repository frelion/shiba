...
/// Whether the surrounding PostgreSQL transaction reached the durable commit
/// boundary for a step result.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum TransactionResult {
    /// The checkpoint CAS and all state/output/continuation writes committed.
    Committed,
    /// No operator writes were attempted; the scheduler only observed Idle or
...
pub(crate) struct StepExecution {
    ...
    pub(crate) transaction: TransactionResult,
}
...
impl StepExecution {
    pub(crate) const fn new(outcome: StepOutcome, usage: WorkUsage) -> Self {
        Self {
            outcome,
            usage,
            transaction: TransactionResult::Committed,
        }
    }

    pub(crate) const fn empty(outcome: StepOutcome) -> Self {
        Self::not_committed(outcome)
    }

    pub(crate) const fn not_committed(outcome: StepOutcome) -> Self {
        Self {
            outcome,
            usage: WorkUsage {
                input_rows: 0,
            ...
            transaction: TransactionResult::NotCommitted,
        }
    }

    pub(crate) fn validate(self) -> Result<(), String> {
        match (self.transaction, self.outcome) {
            (TransactionResult::Committed, StepOutcome::Progress | StepOutcome::Yield) => Ok(()),
            (TransactionResult::NotCommitted, StepOutcome::Idle | StepOutcome::Blocked) => Ok(()),
            (TransactionResult::Committed, outcome) => Err(format!(
                "committed step returned non-committed outcome {outcome:?}"
            )),
            (TransactionResult::NotCommitted, outcome) => {
                Err(format!("non-committed step returned {outcome:?}"))
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn transaction_result_separates_commit_from_admission_outcomes() {
        assert_eq!(
            StepExecution::new(StepOutcome::Yield, WorkUsage::default()).transaction,
            TransactionResult::Committed
        );
        assert_eq!(
            StepExecution::not_committed(StepOutcome::Blocked).transaction,
            TransactionResult::NotCommitted
        );
        assert_eq!(
            StepExecution::empty(StepOutcome::Idle).transaction,
            TransactionResult::NotCommitted
        );
        assert!(
            StepExecution::new(StepOutcome::Progress, WorkUsage::default())
                .validate()
                .is_ok()
        );
        assert!(StepExecution::not_committed(StepOutcome::Idle)
            .validate()
            .is_ok());
    }
}
