...
fn validate_internal_page(facts: PrimitiveFacts, permits_input: bool) -> Result<(), String> {
...
fn validate_no_output(facts: PrimitiveFacts) -> Result<(), String> {
    if facts.output != OutputFacts::None
        || facts.usage.input_rows != 0
        || facts.usage.input_bytes != 0
        || facts.usage.output_rows != 0
...
fn validate_one_effect(facts: PrimitiveFacts) -> Result<(), String> {
...
fn validate_continuation_count(
    ...
    if facts.continuation_rows != u64::from(has_continuation) {
        return Err("Aggregate checkpoint disagrees with its continuation row".into());
    }
    Ok(())
}
...
struct AggregateFields {
    ...
    group_queue_id: Option<i64>,
    aggregate_ordinal: Option<i32>,
    emit_leg: Option<i16>,
...
#[derive(Clone, Debug)]
struct AggregateOrderKey<'a> {
    expression: &'a SortGroupExpr,
    column: String,
}

fn aggregate_effective_order<'a>(
...
fn aggregate_null_safe_equality(left: &str, right: &str, equality: &str) -> String {
    format!(
        "(
...
pub(crate) fn provision(
    ...
    let input = super::storage::payload(client, input_streams[0])?;
...
fn aggregate_nonnegative(value: i64) -> Result<u64, String> {
...
mod tests {
    ...
    use crate::logical::model::{BindingId, ScalarExpr, SlotId, SlotType};
    ...
    fn aggregate_key_equality_uses_the_resolved_operator_and_explicit_nulls() {
        let equality =
            aggregate_null_safe_equality("left.key", "right.key", "OPERATOR(pg_catalog.=)");
